...
| `create-collision` | The greenfield create plan's table name or a claimed index/constraint-index name is occupied. Nothing runs; re-derive the plan against the live catalog to see what holds the name, then drop or rename the occupant or name the constraint's index explicitly in the desired file — re-planning alone reproduces the refusal. Catalog absence checks handle existing occupants; duplicate-name SQLSTATEs remain the race backstop. |
