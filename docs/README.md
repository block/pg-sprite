...
| [supabase.md](supabase.md) | Use pg-sprite with your Supabase app: connections, tested changes, and what comes next |
...
| [refusal-classes.md](refusal-classes.md) | The **refusal routing contract** — the five machine-readable classes above existing reasons, the owner vocabulary for work outside an online engine, the complete map keyed on typed causes where they exist and on sites where they do not, and the proof-type enforcement that makes an unclassified refusal unrepresentable. |
| [lock-budgeted-passthrough.md](lock-budgeted-passthrough.md) | The **lock-budgeted passthrough contract** — the explicit `--accept-blocking` acknowledgement, typed refusal eligibility, required engine-owned lock and statement budgets, marked verdict and exit code, interruption semantics, and orchestrator boundary. |
...
| [capabilities-contract.md](capabilities-contract.md) | The **machine-readable capabilities contract** — the authoritative YAML source, typed row schema, generated-Markdown boundary, embedded CLI JSON shape, versioning, refusal-class mapping, and regenerate-and-diff CI gate. Read this when changing how the support matrix is stored, rendered, or queried. |
