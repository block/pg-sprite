...
| [cli-output-examples.md](cli-output-examples.md) | **CLI output examples** — one real, captured output per shape the CLI produces: the human dry-run report, the JSON plan report for every disposition (execute, safer-sequence substitution, rewrite-required, backend-unavailable, refusal, destructive), exit codes, and the linter in both text and JSON. |
