...
| table name (copy-and-swap target) | copy-and-swap preflight (declaration exists; the validating passage is planned) | `CopySwapTarget` — the copy-and-swap route's proof, carrying today the integer-family PK and the owner role that shadow objects are created under and, once the passage lands, `PreflightedTable`'s facts plus the [v1 scope](copy-and-swap-design.md#v1-scope) checks (integer-family PK, replica identity `DEFAULT` or `FULL`, no OID-bound dependents, derived names within the 63-byte identifier limit, logical decoding and slot/disk headroom); the shadow builder will accept **only** this type | ST-6, RF-1..RF-3 for the copy-and-swap route |
