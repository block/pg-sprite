...
| Live tables with no desired file | Not in `diff`'s view: the diff is single-table scoped, so it never plans or executes a `DROP TABLE`, and the imperative front door refuses the statement as unsupported. Under a declarative model the only convergence for an undeclared table is a drop, so a whole-schema owner (an orchestrator, or an operator running `diff` per file) must enumerate the live tables itself and treat each undeclared one as a blocked destructive change — declared, or dropped through a reviewed process — rather than as up to date. |
