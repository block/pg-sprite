...
func checkRowSecurityPrivileges(ctx context.Context, tx pgx.Tx, schema, table string) error {
 ...
 WHERE n.nspname = $1 AND c.relname = $2`, schema, table).Scan(&owner, &createSchema)
	...
	if !owner {
		return fmt.Errorf("row security on %s requires owner privileges: %w: %w", target, ErrRowSecurityRefused, ErrRowSecurityPrivileges)
	}
	if !createSchema {
		return fmt.Errorf("row security on %s requires CREATE on the database for scratch inspection: %w: %w", target, ErrRowSecurityRefused, ErrRowSecurityPrivileges)
	}
	return nil
}
