...
func TestImplicitRelationNamesMatchServer(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unique with INCLUDE columns", sql: "CREATE TABLE t (a int, b int, c int, UNIQUE (a) INCLUDE (b, c))"},
		{name: "primary key with INCLUDE columns", sql: "CREATE TABLE t (a int, b int, PRIMARY KEY (a) INCLUDE (b))"},
		{name: "exclusion constraint with INCLUDE columns", sql: "CREATE TABLE t (a int, b int, EXCLUDE USING btree (a WITH =) INCLUDE (b))"},
		{name: "exclusion constraint repeating a column", sql: "CREATE TABLE t (a int, EXCLUDE USING btree (a WITH =, a WITH =))"},
		{name: "exclusion constraint over repeated expressions", sql: "CREATE TABLE t (a int, EXCLUDE USING btree ((a + 1) WITH =, (a + 2) WITH =, (a + 3) WITH =))"},
		{name: "identity sequence with SEQUENCE NAME", sql: "CREATE TABLE t (id bigint GENERATED ALWAYS AS IDENTITY (SEQUENCE NAME custom_seq))"},
		{name: "identity sequence with other options", sql: "CREATE TABLE t (id bigint GENERATED BY DEFAULT AS IDENTITY (START WITH 10 INCREMENT BY 5))"},
		{name: "long column names in a unique constraint truncate the generated name", sql: fmt.Sprintf("CREATE TABLE t (%s int, %s int, UNIQUE (%[1]s, %[2]s))", strings.Repeat("b", 40), strings.Repeat("c", 40))},
	}
