...
and `statement_timeout`; a `search_path` that lists `pg_catalog` explicitly has
that entry removed so a user schema can never shadow the catalog, and every
other entry is left as configured). Only `migrate` without `--dry-run` ever
commits a change — every other command is read-only or fully offline.
