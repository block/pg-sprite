...

{
    ...
    echo "#   <migration-prefix> <start>-<end> <execute|refuse:<reason>:<class>|psql>"
    echo "#"
    echo "# expected:"
    echo "#   execute          require exit 0 and outcome executed-natively —"
    echo "#                    pg-sprite itself mutates the database"
    echo "#   refuse:<reason>:<class>  require exit 2 and exactly the engine-emitted"
    echo "#                            <reason> and <class>, then apply"
