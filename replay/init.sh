...

{
    ...
    echo "#   <migration-prefix> <start>-<end> <execute|refuse:<reason>:<class>[:<owner>]|psql>"
    echo "#"
    echo "# expected:"
    echo "#   execute          require exit 0 and outcome executed-natively —"
    echo "#                    pg-sprite itself mutates the database"
    echo "#   refuse:<reason>:<class>[:<owner>]"
    echo "#                    require exit 2 and exactly the engine-emitted <reason>,"
    echo "#                    <class>, and <owner>, then apply the same statement via"
    echo "#                    psql to advance. <owner> is named exactly when <class> is"
    echo "#                    no-online-safety-problem; only capability-boundary,"
    echo "#                    no-online-safety-problem, and by-design are pinnable"
