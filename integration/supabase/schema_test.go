...
func TestDesiredSupabaseObjects(t *testing.T) {
	pool := fixture(t)
	execSQL(t, pool, "CREATE EXTENSION IF NOT EXISTS citext WITH SCHEMA extensions")
	for _, tc := range []struct{ name, owner, extra string }{
		{"auth_default", "DEFAULT auth.uid()", ""},
		{"extension_type", "", ", email extensions.citext"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			name := "pgsprite_desired_" + tc.name
			table := newTable(t, pool, name)
			ds, err := statement.ParseDesired(fmt.Sprintf("CREATE TABLE %s (id int PRIMARY KEY, owner_id uuid NOT NULL %s, body text NOT NULL%s)", name, tc.owner, tc.extra))
			...
			case "auth_default":
				...
				require.NoError(t, pool.QueryRow(t.Context(), "SELECT column_default FROM information_schema.columns WHERE table_schema='public' AND table_name=$1 AND column_name='owner_id'", name).Scan(&expression))
				assert.Equal(t, "auth.uid()", expression)
			case "extension_type":
				var schema, typ string
				require.NoError(t, pool.QueryRow(t.Context(), "SELECT udt_schema,udt_name FROM information_schema.columns WHERE table_schema='public' AND table_name=$1 AND column_name='email'", name).Scan(&schema, &typ))
...
func TestSupabaseRefusalsLeaveSchemaUnchanged(t *testing.T) {
	pool := fixture(t)
	t.Run("text rewrite", func(t *testing.T) {
