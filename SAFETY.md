...
| `pkg/schemachange` — shadow builder, orchestrator, **cutover swap + fidelity gate** | ✅ core | shadow builder exists (`BuildShadow` → `BuiltShadow`, `SourceOfDerivedName`); the orchestrator leaf adds the `*dbconn.TableLockSession` parameter, `DropShadow`, and `InspectShadow`; cutover planned | LK-2, ST-5, ST-6, ST-7 (shadow build); LK-1 at the orchestrator (planned); LK-4 and RF-1..RF-6 at cutover (planned) |
