...
| `pkg/schemachange` — shadow builder, orchestrator, **cutover swap + fidelity gate** | ✅ core | shadow lifecycle exists (`BuildShadow` → `BuiltShadow`, `DropShadow`, `InspectShadow`, `SourceOfDerivedName`), every operation taking the `*dbconn.TableLockSession` it runs under; copier and cutover planned | LK-1, LK-2, ST-5, ST-6, ST-7 (shadow build, drop, inspect); LK-4 and RF-1..RF-6 at cutover (planned) |
