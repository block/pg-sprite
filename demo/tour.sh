...
dry_run() {
    ...
    if [ "$CHECK" = 1 ]; then
        ...
        assert_eq "plan format_version of [$sql]" 3 "$(jq -r '.format_version' <<<"$out")"
...
# diff_refused desired_file cause
#
# A greenfield desired file whose create shape the create path refuses: the
# plan exits with the refusal code 2, the aggregate disposition is refuse, and
# the refused statement carries the create path's typed cause
...
diff_refused() {
    ...
    if [ "$CHECK" = 1 ]; then
        ...
        "$PGS" diff --url "$PG_DSN" --desired "$desired" --schema public \
            || echo "(refused, exit $? — expected: the create path refuses this shape)"
    fi
}
...
run_diff() {
    ...
    diff_refused desired/gadgets_if_not_exists.sql if-not-exists
}
